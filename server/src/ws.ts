...
import {
  ...
  CH_BOARDS, CH_PROJECT_WORKFLOW, CH_DOCS, CH_CALENDAR_REMINDER, CH_CALENDAR_EVENTS, CH_DOC_MENTION,
  publish,
...
export async function resolveWsEventRecipientUserIds(
  ...
  const targetedUserIds = event.type === 'doc.mention'
    ...
    : event.type === 'calendar.reminder'
      ...
      : event.type === 'conversation.dissolved'
        ? event.recipientUserIds
      : null
  if (targetedUserIds) {
...
export function attachWebSocket(httpServer: Server) {
  ...
  const forward = async (channel: string, payload: string) => {
    ...
    const current = previous.catch(() => {}).then(async () => {
      ...
      for (const c of clients) {
        ...
        try { c.ws.send(outboundPayload) } catch { /* ignore */ }
      }
    })
  ...
  const onBroadcast = (channel: string, payload: string) => {
    void forward(channel, payload).catch((error) => console.warn('[ws] broadcast failed', error))
  }
