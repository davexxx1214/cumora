...
更新：2026-08-29。**源码已实现，尚未线上启用；不是生产就绪声明。** 当前检查结果及未完成项见 [实施计划](../plan.md#16-执行记录共享文件实现2026-08-29)。
...
7. 启用 API 文件开关，再启动受控 daemon；先用专用测试群/项目走通浏览器上传→Agent 修改→浏览器下载→撤权/切换。迁移、重启、上线需要单独记录，当前尚未执行。
...
不是完整 POSIX/协同编辑系统；不自动合并 Office 文件、不自动索引、不接 Git、不提供跨群共享或公开下载。FUSE 文件内容使用有上限的直接 I/O 缓冲；最大目录与版本规模的性能尚未完成测量。先前重复运行问题已经定位并完成 20 轮真实挂载复测；新增的 daemon 消失和 25MiB HTTP 边界集成项仍须在 Linux 主机恢复后运行。服务启用前还需要完成部署及线上灰度记录。
